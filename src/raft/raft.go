...
func (rf *Raft) SendHeartbeats() {
	...
	for idx, _ := range rf.peers {
		...
		entries := rf.logs[rf.nextIndex[idx]:]    // TODO: heartbeat's entry should be determined by nextIndex?
		...
		for { // since ultimately there will be election timeout, so this goroutine eventually quits
			...
			for i, status := range status {
				if status == FAIL { // if RPC failed, then retry this RPC until it succeed
					...
					_ = i
					// TODO: do we need to resend failed RPC? Maybe next heartbreak will do?
				}
			}
			statusMutex.Unlock()
...
func (rf *Raft) SendAppendEntries(server int, args AppendEntriesArgs, reply AppendEntriesReply, result *int, statusMutex *sync.Mutex) {
	log.Printf("Raft server %d called SendAppendEntries with prevLogIndex %d\n", rf.me, args.PrevLogIndex)
	...
	if rf.matchIndex[server] != len(rf.logs)-1 { // if not matched, keep sending AppendEntries to force it to match
		...
		args = AppendEntriesArgs{term, leaderId, prevLogIndex, prevLogTerm, entries, leaderCommit}
		reply = AppendEntriesReply{-1, false}                          // if you see -1 in reply, then the receiver never receives the RPC
		rf.SendAppendEntries(server, args, reply, result, statusMutex) // use recursion to resend AppendEntries RPC
		// TODO: something wrong with resend, why prevLogIndex has not changed
	}
}
...
func (rf *Raft) RunElection() {
	...
	// TODO: RunElection must be blocked here, since [server0] has been elected as leader isn't printed
...
//
// only called when leader send AppendEntries RPC (if heartbeat, no new logs appended so no need to commit new logs, too)
...
// return when find this N (I think if it doesn't return, then it does no harm, too)
//
