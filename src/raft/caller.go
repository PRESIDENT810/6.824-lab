...
func (rf *Raft) SendHeartbeats(term int) {
	...
	for server, _ := range rf.peers {
		...
		go rf.PrepareAppendEntries(term, server, true)
	}
}
...
func (rf *Raft) RequestReplication(term int) {
	...
	for server, _ := range rf.peers {
		...
		go rf.PrepareAppendEntries(term, server, false)
	}
}

func (rf *Raft) PrepareAppendEntries(term int, server int, heartbeat bool) {
	rf.mu.Lock()
	defer rf.mu.Unlock()
...
func (rf *Raft) SendAppendEntries(server int, args AppendEntriesArgs, reply AppendEntriesReply) {
	...
	if reply.Ignore {
		Printf("AppendEntries from LEADER %d to FOLLOWER %d [RPC %d] ignored due to being outdated\n", rf.me, server, args.RPCID)
		return
	}
	...
	if reply.Success { // this follower's log now is matched to mine, so update the nextIndex and matchIndex
		...
		if rf.nextIndex[server]-1 == args.PrevLogIndex { // no one else has decremented nextIndex
			...
			return // the one changed my nextIndex should take care of the following synchronization
		}
	}
...
func (rf *Raft) SendInstallSnapshot(server int, args InstallSnapshotArgs, reply InstallSnapshotReply) {
	...
	if reply.Ignore {
		Printf("InstallSnapshot from LEADER %d to FOLLOWER %d [RPC %d] ignored due to being outdated\n", rf.me, server, args.RPCID)
		return
	}
